...
export async function generateMetadata({ params }) {
  ...
  const title = `Matrimoniale ${location} | Destiny`;
  const description = `Descoperă matrimoniale pentru ${location} - cele mai potrivite profiluri de compatibilitate și conexiuni autentice.`;
  
  return {
    title,
    description,
    alternates: {
      canonical: `${siteURL}/matrimoniale/${location}`,
      languages: {
        "ro-RO": `/ro-RO/matrimoniale/${location}`,
      },
    },
    robots: {
      index: true,
    ...
    const translatedLinks = {
      ...
      contText: await fetchTranslation("Cont", targetLanguage),
         home: await fetchTranslation("Acasa", targetLanguage),
          ...
          // Preluăm utilizatorii de pe server (din colecția "Users")
  const usersCollection = collection(db, "Users");
  const userSnapshot = await getDocs(usersCollection);
  const users = userSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

  // Exemplu: filtrăm utilizatorii care au datele de "responses" (conform logicii din codul MyCourses)
  const validUsers = users.filter(user => user.afisabil);
  return (
    <>
      <Preloader />
      <Header
      ...
      <div className="main-content overflow-hidden">
        {/* Transmitem ca prop valoarea din URL (params.id) către componenta Students */}
        <Students location={params.id} validUsers={validUsers}/>
      </div>
    </>
  );
}
